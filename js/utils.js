const anzhiyu = {
  ...
  sayhi: function () {
    ...
    const getTimeState = () => {
      ...
      if (hour >= 0 && hour <= 5) {
        message = "ゆっくり眠り、元気を保ちましょう！";
      } else if (hour > 5 && hour <= 10) {
        message = "朝が大事です！";
      } else if (hour > 10 && hour <= 14) {
        message = "しっかり食べて仕事に力を出しましょう！";
      } else if (hour > 14 && hour <= 18) {
        message = "集中して難関を突破しましょう！";
      } else if (hour > 18 && hour <= 24) {
        message = "疲れすぎないで、早寝が健康に良いです！";
      }
  ...
  categoriesBarActive: function () {
    ...
    if (urlinfo === "/") {
      $categoryBar.querySelector("#ホームページ").classList.add("select");
    } else {
