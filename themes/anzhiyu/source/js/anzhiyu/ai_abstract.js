(function () {
  ...
  async function readAloud() {
    if (!summaryID) {
      anzhiyu.snackbarShow("要約はまだ読み込み中です。少々お待ちください。。。");
      return;
    }
    ...
    try {
      ...
      if (response.status === 403) {
        console.error("403 referとkeyが一致しません。");
      } else if (response.status === 500) {
        console.error("500 システム内部エラー");
      } else {
  ...
  function startAI(str, df = true) {
    ...
    elapsed = 0;
    observer.disconnect();
    explanation.innerHTML = df ? "生成中. . ." : "お待ちください. . .";
    aiStr = str;
  ...
  async function aiAbstractTianli(num) {
    ...
    try {
      ...
      if (response.status === 403) {
        result = {
          summary: "403 referとkeyが一致しません。",
        };
      } else if (response.status === 500) {
        result = {
          summary: "500 システム内部エラー",
        };
      } else {
      ...
      if (summary) {
        ...
        startAI("要約の取得に失敗しました!!!Tianliサービスが正常かどうか確認してください!!!");
      }
      clearInterval(animationInterval);
    } catch (error) {
      console.error(error);
      explanation.innerHTML = "例外が発生しました" + error;
    }
  }
  ...
  function recommendList() {
    ...
    if (!thumbnail.length) {
      ...
      return `申し訳ありませんが、類似の記事が見つかりませんでした。本サイトの最新記事をご覧ください：<br /><div class="ai-recommend">${list}</div>`;
    }

    let list = "";
    for (let i = 0; i < thumbnail.length; i++) {
      const item = thumbnail[i];
      list += `<div class="ai-recommend-item"><span>おすすめ${
        i + 1
      }：</span><a href="javascript:;" onclick="pjax.loadUrl('${item.href}')" title="${
        item.title
      }" data-pjax-state="">${item.title}</a></div>`;
    ...
    return `おすすめ記事：<br /><div class="ai-recommend">${list}</div>`;
  }

  function aiGoHome() {
    startAI("ブログのホームページに移動しています...", false);
    timeouts[2] = setTimeout(() => {
      if (window.pjax) {
        pjax.loadUrl("/");
      } else {
  ...
  function introduce() {
    if (mode == "tianli") {
      startAI("私は記事補助AI：TianliGPTです。下のボタンをクリックして、この記事の概要を生成したり、関連記事を推薦したりできます。");
    } else {
      startAI(`私は記事補助AI：${gptName} GPTです。下のボタンをクリックして、この記事の概要を生成したり、関連記事を推薦したりできます。`);
    }
  }
  ...
  function onAiTagClick() {
    if (mode === "tianli") {
      ...
      startAI(
        "こんにちは、私はTianliが開発した要約生成アシスタントTianliGPTです。GPT-4ベースの生成AIです。ここでは要約の事前生成と表示のみを担当しており、直接対話することはできません。このようなAI要約インターフェースが必要な場合は、下記から購入できます。"
      );
    } else {
      post_ai.querySelectorAll(".ai-btn-item").forEach(item => (item.style.display = "block"));
      document.getElementById("go-tianli-blog").style.display = "none";
      startAI(
        `こんにちは、私は本サイトの要約生成アシスタント${gptName} GPTです。GPT-4ベースの生成AIです。ここでは要約の事前生成と表示のみを担当しており、直接対話することはできません。`
      );
    }
  }
